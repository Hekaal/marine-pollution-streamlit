...
with col1:
    ...
    if not filtered_df.empty:
        ...
        st.plotly_chart(fig_map, use_container_width=True) # Added missing plotting call
...
if not filtered_df.empty:
    if 'aware_ans' in filtered_df.columns:
        ...
        if not aware_count.empty:
            ...
            st.plotly_chart(fig_awareness, use_container_width=True) # Added missing plotting call
...
if not filtered_df.empty:
    ...
    st.info("Tabel data tidak dapat ditampilkan karena tidak ada data yang difilter.") # Corrected typo here
st.markdown("---")
st.markdown(
    "<center><sub>🐬 Dibuat untuk meningkatkan kesadaran akan pentingnya menjaga laut 🌊</sub></center>",
    unsafe_allow_html=True
)
st.sidebar.markdown("---")
