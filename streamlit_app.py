...
with col2:
    ...
    if not filtered_df.empty:
        ...
        if not top_pollution.empty:
            fig_bar = px.bar(x=top_pollution.index, y=top_pollution.values, labels={'x': 'Jenis Polusi', 'y': 'Jumlah Kejadian'}, title="Top 10 Jenis Polusi", color_discrete_sequence=px.colors.qualitative.Pastel)
...
st.header("\U0001F4C8 Tren Waktu Insiden Polusi")
...
if not filtered_df.empty:
    ...
    st.download_button("📥 Download Data yang Difilter (.csv)", data=csv, file_name='filtered_marine_pollution.csv', mime='text/csv')
else:
